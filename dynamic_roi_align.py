...
class DynamicRoIAlign(torch.nn.Module):
    ...
    def __init__(self, spatial_scale: Tuple[int, int]=(640, 640), sampling_ratio=-1, aligned=False):
    ...
    def forward(self, input_feature_map: torch.Tensor, rois: torch.Tensor, output_height: Optional[torch.Tensor | int], output_width: Optional[torch.Tensor | int]) -> torch.Tensor:
...
if __name__ == '__main__':
    ...
    output = roi_align_module.forward(input_feature_map, rois, output_height, output_width)
    ...
    torch.onnx.export(
        ...
        opset_version=17  # Opset 16+ required for grid_sample with dynamic shapes
    )
